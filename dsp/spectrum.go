...
func (sp *Spectrum) Recalculate(bins int) int {
	...
	// for xB := 0; xB <= bins; xB++ {
	// 	var fbL = sp.freqToIdx(math.Pow(10.0, (float64(xB)*bandWidth)+minLog*1.05), math.Floor)
	...
	// 	}
	// }
	// 	var f = sp.idxToFreq(idx)
	// 	var pos = int(math.Floor(bandWidth * (math.Log10(f) - minLog)))

	// 	fmt.Println(pos)

	// 	if pos > last {
	// 		lastB++
	// 		sp.bins[lastB].floorFFT = idx
	// 		sp.bins[lastB].ceilFFT = idx + 1
	// 		last = pos
	// 	} else {
	// 		sp.bins[lastB].ceilFFT = idx + 1
	// 	}
	...
	// for xB := 0; xB <= bins; xB, m = xB+1, m+bandWidth {
	// 	var f = math.Pow(10, m)
	// 	var idx = sp.freqToIdx(f, math.Round)

	// 	sp.bins[xB].floorFFT = idx
	// }
